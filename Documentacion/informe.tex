...
\begin{itemize}
    ...
    \item \textbf{Rendimiento de Interfaz (Tkinter):} El motor de renderizado de la UI de Tkinter no est\'a dise\~nado para procesar y redibujar flujos de video de alta definici\'on o alto frame rate de manera eficiente en la CPU.
\end{itemize}
...
Esto instalar\'a:
\begin{itemize}
    \item \textbf{Pillow:} Para el procesamiento y renderizado de frames de imagen en Tkinter.
...
Adicionalmente, se requieren dependencias a nivel de sistema operativo para el soporte de audio y video:
\begin{itemize}
    \item \textbf{En Linux (Debian/Ubuntu):}
    Ejecute el siguiente comando para instalar el soporte de Tkinter, PortAudio (requerido por \textit{sounddevice}) y FFmpeg (como alternativa de video):
    \begin{lstlisting}
sudo apt update
